...
try {
  for (const width of [320, 375, 768, 1440]) {
    ...
    await page.locator(".creator-character").waitFor({ state: "visible" });
    await page.evaluate(async () => { await Promise.all([...document.images].map((image) => image.decode())); });
    const measured = await page.evaluate(() => {
      const github = document.querySelector(".hero-actions button");
      ...
      const scene = document.querySelector(".creator-scene").getBoundingClientRect();
